...
var cargarMunicipios = function (prov) {
    $.ajax({
        type: 'GET',
        url: 'https://aiongest.internetinfraestructuras.es/apiuuii.php',
        url: 'http://localhost:8080/apiuuii.php',
        contentType: "text/plain",
        dataType: 'json',
        data: {
        ...
        success: function (data) {
            myJsonData = data;
            //console.log("MUNICIPIOS", myJsonData.data);
...
var tabPagos = function () {
    ...
    $('#numCuenta3').on('input', function () {
        iban = $('#accountCode').val() + $('#accountEntity').val() + $('#accountOffice').val() + $('#numCuenta1').val() + $('#numCuenta2').val() + $('#numCuenta3').val();
        if (validarIban(iban)) {
            $('#labelIban').hide();
            ibanValido = true;
        } else {
...
var checkTabPagos = function () {
    ...
    // if (ibanValido && direccionInstalacionValida && tarj) {
    //     if (checkNuevaDireccion) {
    //         if (direccionFacturacionValida) {
    //             if (checkPrestacion) {
    //                 $('#btnConfirmarCompra').prop('disabled', false);
