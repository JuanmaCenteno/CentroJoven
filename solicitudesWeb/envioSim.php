...
$emailDestino = filter_input(INPUT_POST, 'email', FILTER_SANITIZE_STRING);
$nombre = filter_input(INPUT_POST, 'nombre', FILTER_SANITIZE_STRING);
$dni = $_POST['dni'];
$agencia = $_POST['agencia'];
$plataforma = filter_input(INPUT_POST, 'plat', FILTER_SANITIZE_STRING);
...
$url = "https://www.genei.es/json_interface/crear_envio"; // ENVÍO SANDBOX
...
error_log("JSON " . json_encode($fields));
...
error_log("ARRAY RESULTADO " . print_r($arr,true));
//print_r($arr);
error_log(strval($arr['estado']) . " | " . $arr['codigo_envio']);
if (strval($arr['estado']) == "1") { // ENVÍO CORRECTO
    $tr = $arr['codigo_envio'];
    //echo "OK";
    error_log("UPDATE solicitudesweb SET ID_TRANSPORTE = " . $tr . ", ESTADO = 2 WHERE ID_TRANSPORTE = " . $idTransporte);
