...
if ($codigo == "") {
    ...
    // Inserción Solicitud
    $query = "INSERT INTO multiplataforma.solicitudesweb VALUES(null,2,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,6,null,null,?,?,?,?)";
    $stmt = $db->prepare($query);
    $stmt->bind_param('ssssssssssssssssssssssssssss', $idTransporte, $idTarifa, $nombreTarifa, $precioTarifa, $numeroDocumento, $nombreRazonSocial, $nombreTitular, $apellidosTitular, $fechaNacimientoTitular, $numeroTelefonoTitular, $emailTitular, $alta, $numeroTelefonoNuevo, $numeroICCNuevo, $titularCuentaBancaria, $iban, $direccionEnvio, $codigoPostal, $municipio, $provincia, $direccionFacturacion, $codigoPostalDireccionFacturacion, $municipioDF, $provinciaDF, $numTarjeta, $fechaCaducidad, $numCcv, $datetime->format('Y-m-d H:i:s'));
    $stmt->execute();
    if ($stmt) {
        ...
        if (sizeof($resultado) > 0) { // CLIENTE EXISTE
            ...
            $stmt = $db->prepare($query);
            $stmt->bind_param('ssssssssssssssssssss', $nombreTitular, $apellidosTitular, $numeroDocumento, $tipoDocumento, $direccionFacturacion, $localidad, $provincia, $comunidad, $iban, $codigoPostalDireccionFacturacion, $numeroTelefonoNuevo, $numeroTelefonoTitular, $emailTitular, $hoy, $hoy, $nota, $fechaNacimientoTitular, $numTarjeta, $fechaCaducidad, $numCcv);
            $stmt->execute();
            if ($stmt) {
    ...
    // Código Envío SMS e Email 64 MOVIL
